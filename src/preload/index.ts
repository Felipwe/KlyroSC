...
const api: KlyroApi = {
  ...
  social: {
    ...
    kickFromJam: (userId) => invoke(IPC.socialJamKick, userId),
    transferJam: (userId) => invoke(IPC.socialJamTransfer, userId),
    ...
    onChatRejected: (cb) =>
      listen<{ friendId?: string; tempId?: string; code: string }>(IPC.socialChatRejected, cb)
  },
  events: {
